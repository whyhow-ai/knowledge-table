...
from app.services.loaders.unstructured_service import UnstructuredLoader, UnstructuredDependencyError

@pytest.fixture
def mock_unstructured_file_loader():
    with patch('app.services.loaders.unstructured_service.UnstructuredFileLoader') as mock:
        yield mock
...
async def test_load_method(mock_unstructured_file_loader):
    ...
    mock_documents = [
        Document(page_content="Test content 1", metadata={"source": file_path}),
        Document(page_content="Test content 2", metadata={"source": file_path})
    ]
    mock_unstructured_file_loader.return_value.load.return_value = mock_documents
    ...
    mock_unstructured_file_loader.assert_called_once_with(file_path, unstructured_api_key=None)
...
async def test_load_and_split_method(mock_unstructured_file_loader):
    ...
    mock_documents = [
        Document(page_content="Test content 1", metadata={"source": file_path}),
        Document(page_content="Test content 2", metadata={"source": file_path}),
        Document(page_content="Test content 3", metadata={"source": file_path})
    ]
    mock_unstructured_file_loader.return_value.load_and_split.return_value = mock_documents
...
async def test_load_with_api_key(mock_unstructured_file_loader):
    ...
    mock_unstructured_file_loader.assert_called_once_with(file_path, unstructured_api_key=api_key)